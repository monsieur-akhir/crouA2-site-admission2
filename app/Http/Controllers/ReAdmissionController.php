...
use App\Http\Requests\ReadmissionRequestUpdate;
...
class ReAdmissionController extends Controller
{
    ...
    public function all(Request $request)
    {
        $demandes =  DB::table('re_demande')
            ...
            ->get();
        return view('layouts.traiter-readmission',[
            'demandes' =>$demandes
        ]);
    }
    ...
    // public function readmissonDemande(Request $request)
    public function readmissonDemande(ReadmissionRequest $request)
    {
        // $validated = $request->validated();
        $file = $request->file('image');
        $filename = date('YmdHi').$file->getClientOriginalName();
       /*  $file-> move(public_path('photos_readmission'), $filename);
        $data['image']= url('photos_readmission/'.$filename) ;
         $file-> move('photos_readmission', $filename);
        $data['image']= 'photos_readmission/'.$filename ; */
        $file = $request->file('image');
        $filename = date('YmdHi').$file->getClientOriginalName();
        $file->move('photos_readmission', $filename);
       ...
       // $file = $request->file('image');
        ...
        $demande->statut = 0;
        $demande->cite_id = $request->cite_id;
        $demande->batiment_id = $request->batiment_id;
        $demande->palier_id = $request->palier_id;
        $demande->chambre_id = $request->chambre_id;
        $demande->lit_id = $request->lit_id;
        $demande->save();
        $pdf = PDF::loadView('dossiers.fiche-readmission',[
            "data"=>$demande
        ]);
       /*  Mail::send([], [], function ($message) use ($demande,$pdf) {
            $attachment1 = new Swift_Attachment($pdf->output(), "fiche-readmission.pdf",'application/pdf');
            $attachment2 = Swift_Attachment::fromPath(public_path() . "/documents/FICHE_DENGAGEMENT.pdf");
            $message->to($demande->email_etudiant)
              ->subject("Demande de readmission de chambre")
              ->setBody('Bonjour votre demande a été prise en compte')
              ->attach($attachment1)
              ->attach($attachment2);
          }); */
        return view('layouts.success-readmission',[
            "demande"=>$demande
        ]);
    }
    ...
    public function trouverFiche(Request $request)
    {
        ...
        $batiments= batiment::all();
        if ($demande) {
            return view('layouts.update-readmission',[
                'demande' =>$demande,
                "niveaux"=>$niveaux,
                "departements"=>$departements,
                "universites"=>$universites,
    ...
    public function updateDemande(ReadmissionRequestUpdate $request)
    {
        $validated = $request->validated();
        $demande = ReAdmission::where("matricule_crou",$request->matricule_crou)->first();
        $file = $request->file('image');
        if($file){
            $file_base64 = 'data:'.$file->getClientMimeType().';base64,'.base64_encode(file_get_contents($request->file('image')));
            $demande->image = $file_base64;
        }
        $demande->genre = $request->genre;
        $demande->num_carte = $request->num_carte;
        $demande->nom_etudiant = $request->nom_etudiant;
    ...
    public function detailsDemande(Request $request)
    {
        ...
        $demande = ReAdmission::where("matricule_crou",$request->mat)
        ->with(['chambre_attribue'])
        ->first();
        // dd($demande);
        return view('layouts.details-demande-readmission',[
            'demande' =>$demande,
            'lits' =>$lits,
        ]);
    }
