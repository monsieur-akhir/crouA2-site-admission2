                        ...
                        <div class="form-group col-md-12">
                          ...
                          <select class="form-control select2" style="width: 100%" name="chambre_id" id="chambre_id" required>
                            ...
                            <option value="{{ $chambre->id }}">
                              {{ $chambre->libelle }} -
                              {{ $chambre->palier->libelle }} -
                              {{ $chambre->palier->batiment->libelle }} -
                              {{ $chambre->palier->batiment->cite->libelle }}
                            </option>
                            @endforeach
                          </select>
              ...
              <div class="table-responsive pt-3">
                <table id="dataTable" class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $lit->libelle }}</td>
                      <td>{{ $lit->chambre->libelle }}</td>
                      <td>{{ $lit->chambre->palier->libelle }}</td>
                      <td>{{ $lit->chambre->palier->batiment->libelle }}</td>
                      <td>{{ $lit->chambre->palier->batiment->cite->libelle }}</td>
                      <td>
                        <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal{{ $lit->id }}">
                          Modifier
                        </button>
                      </td>
                      <!-- Modal -->
                      <div id="myModal{{ $lit->id }}" class="modal fade" role="dialog">
                            ...
                            <div class="modal-body">
                              <form>
                                ...
                                <div class="row">
                                  <div class="form-group col-md-12">
                                    <label>Chambre</label>
                                    <select class="form-control" name="chambre_id{{ $lit->id }}" id="chambre_id{{ $lit->id }}" required>
                                      @foreach($chambres as $chambre)
                                      <option value="{{ $chambre->id }}" @if($lit->chambre_id == $chambre->id ) {{ 'selected=selected' }} @endif>
                                        {{ $chambre->libelle }} -
                                        {{ $chambre->palier->libelle }} -
                                        {{ $chambre->palier->batiment->libelle }} -
                                        {{ $chambre->palier->batiment->cite->libelle }}
                                  ...
                                  <div class="form-group col-md-12">
                                    <label>Libelle lit</label>
                                    <input required type="text" value="{{ $lit->libelle }}" class="form-control" name="libelle{{ $lit->id }}" id="libelle{{ $lit->id }}">
                                  </div>
                                </div>
                                <div class="modal-footer" style="background-color: white">

                                  <button onclick="updateLit('{{ $lit->id }}')" class="btn btn-primary">Enregistrer</button>
                                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                </div>
